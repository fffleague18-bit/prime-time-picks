    ...
    return (
        <div className="mt-2 text-xs">
            ...
            >
                ...
                <span className="text-slate-700 font-medium truncate">{formattedName}</span>
                {statusConfig.text && (
                ...
                {!statusConfig.spin && (toolCall.arguments_string || results) && (
                    <ChevronRight className={cn("h-3 w-3 text-slate-400 transition-transform shrink-0", 
                        expanded && "rotate-90")} />
                )}
            </button>
...
export default function MessageBubble({ message, currentUser }) {
    ...
    return (
        <div className={cn("flex items-start gap-3", isUser ? "justify-end" : "justify-start")}>
            {!isUser ? (
                <div className="h-8 w-8 rounded-full bg-emerald-500 flex items-center justify-center text-white mt-0.5 shrink-0">
                    <Bot className="h-5 w-5" />
                </div>
            ) : (
                currentUser && <PlayerAvatar icon={currentUser.profile_icon} name={currentUser.display_name} className="w-8 h-8 shrink-0" />
            )}
                    ...
                    <div className={cn(
                        ...
                        <ReactMarkdown 
                            ...
                            components={{
                                ...
                                a: ({ children, ...props }) => (
                                    <a {...props} target="_blank" rel="noopener noreferrer" className="text-emerald-600 hover:underline">{children}</a>
                                ),
